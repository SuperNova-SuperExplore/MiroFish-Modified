...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Catatan internal
  minutesPerRound: {
    type: Number,
    default: 30 // Catatan internal
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Catatan internal
      enable_graph_memory_update: true  // Catatan internal
    }
    ...
    if (res.success && res.data) {
      ...
      startError.value = res.error || 'Gagal menyala'
      addLog(`✗ Gagal menyala: ${res.error || 'Error tidak diketahui'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Error saat menyalakan: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Gagal menghentikan: ${res.error || 'Error tidak diketahui'}`)
    }
  } catch (err) {
    addLog(`Error saat menghentikan: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Semua platform selesai berjalan')
        }
    ...
    console.warn('Gagal mengambil status runtime:', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Gagal mengambil status detail:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Request pembuatan laporan dikirim. Mohon tunggu...')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Buat laporantask dimulai: ${reportId}`)

      // Catatan internal
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Gagal menyalakan pembuatan laporan: ${res.error || 'Error tidak diketahui'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Error pembuatan laporan: ${err.message}`)
    isGeneratingReport.value = false
  }
}

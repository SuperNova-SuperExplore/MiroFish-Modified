  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="Muat ulang graf">
          <span class="icon-refresh" :class="{ 'spinning': loading }">↻</span>
          <span class="btn-text">Refresh</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="Perbesar/kembalikan">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'Memori GraphRAG diperbarui real-time' : 'Update real-time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Masih ada proses kecil di belakang layar. Muat ulang graf beberapa saat lagi.</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="Tutup info">
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>Memuat data graf...</p>
      </div>

      <!-- Catatan UI -->
      <div v-else class="graph-state">
        <div class="empty-icon">❖</div>
        <p class="empty-text">Menunggu ontologi...</p>
      </div>
    </div>
...
const showEdgeLabels = ref(true) // Catatan internal
const expandedSelfLoops = ref(new Set()) // Catatan internal
const showSimulationFinishedHint = ref(false) // Catatan internal
const wasSimulating = ref(false) // Catatan internal
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Catatan internal
}
...
const renderGraph = () => {
  ...
  // Catatan internal
  const edgePairCount = {}
  const selfLoopEdges = {} // Catatan internal
  ...
  // Catatan internal
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Catatan internal
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Catatan internal
        }
      })
      return
    }
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Catatan internal
      const x1 = sx + 8  // Catatan internal
      const y1 = sy - 4
      const x2 = sx + 8  // Catatan internal
      const y2 = sy + 4
      // Catatan internal
    ...
    const offsetRatio = 0.25 + pairTotal * 0.05 // Catatan internal
  ...
  simulation.on('tick', () => {
    ...
    linkLabels.each(function(d) {
      ...
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Catatan internal
    })

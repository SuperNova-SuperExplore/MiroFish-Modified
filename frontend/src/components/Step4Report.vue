            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Sedang membuat {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Masuk interaksi lanjut</span>
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Catatan internal
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Catatan internal
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Catatan internal
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Catatan internal
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Catatan internal
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Catatan internal
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Catatan internal
    const queryMatch = text.match(/Pertanyaan analisis:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Catatan internal
    const reqMatch = text.match(/Skenario prediksi:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Catatan internal
    const factMatch = text.match(/Fakta prediksi terkait:\s*(\d+)/)
    const entityMatch = text.match(/Entitas terkait:\s*(\d+)/)
    const relMatch = text.match(/Rantai relasi:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    // Catatan internal
    const subQSection = text.match(/### Sub-pertanyaan analisis\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // Catatan internal
    const factsSection = text.match(/### [Fakta kunci][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Catatan internal
    const entitySection = text.match(/### [Entitas utama]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const summaryMatch = block.match(/Ringkasan:\s*"?(.+?)"?(?:\n|$)/)
        const relatedMatch = block.match(/Fakta terkait:\s*(\d+)/)
        return {
    ...
    // Catatan internal
    const relSection = text.match(/### [Rantai relasi]\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Catatan internal
    const nodesMatch = text.match(/Total node:\s*(\d+)/)
    const edgesMatch = text.match(/Total edge:\s*(\d+)/)
    const activeMatch = text.match(/Fakta aktif saat ini:\s*(\d+)/)
    const histMatch = text.match(/Riwayat\/Fakta kedaluwarsa:\s*(\d+)/)
    ...
    // Catatan internal
    const activeSection = text.match(/### [Fakta aktif saat ini][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Catatan internal
    const histSection = text.match(/### [Riwayat\/Fakta kedaluwarsa][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Catatan internal
    const entitySection = text.match(/### [Entitas terkait]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // Catatan internal
    const topicMatch = text.match(/\*\*Topik wawancara:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()

    // Catatan internal
    const countMatch = text.match(/\*\*Jumlah narasumber:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Catatan internal
    const reasonMatch = text.match(/### Alasan memilih narasumber\n([\s\S]*?)(?=\n---\n|\n### Transkrip wawancara)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Catatan internal
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Pilih ([^((]+)(?:[((]index\s*=?\s*\d+[))])?[::]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^Tidak dipilih|^Kesimpulan|^Pilihan akhir /)) {
          // Catatan internal
    ...
    // Catatan internal
    const interviewBlocks = text.split(/#### Wawancara #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Catatan internal
      const bioMatch = block.match(/_Ringkasan:\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      // Catatan internal
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Kutipan kunci|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()

        // Catatan internal
        const twitterMatch = answerText.match(/[Jawaban platform Twitter]\n?([\s\S]*?)(?=[Jawaban platform Reddit]|$)/)
        const redditMatch = answerText.match(/[Jawaban platform Reddit]\n?([\s\S]*?)$/)
        ...
        if (!twitterMatch && redditMatch) {
          // Catatan internal
          if (interview.redditAnswer && interview.redditAnswer !== '(Platform ini belum memberi respons)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(Platform ini belum memberi respons)') {
      ...
      // Catatan internal
      const quotesMatch = block.match(/\*\*Kutipan kunci:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
    ...
    // Catatan internal
    const summaryMatch = text.match(/### Ringkasan wawancara dan poin utama\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Catatan internal
    const queryMatch = text.match(/Query pencarian:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Catatan internal
    const countMatch = text.match(/Ditemukan\s*(\d+)\s*item/)
    if (countMatch) result.count = parseInt(countMatch[1])

    // Catatan internal
    const factsSection = text.match(/### Fakta terkait:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    // Catatan internal
    const edgesSection = text.match(/### Edge terkait:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // Catatan internal
    const nodesSection = text.match(/### Node terkait:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Skenario prediksi: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memori kunci saat ini (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entitas utama (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Rantai relasi (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Sub-pertanyaan (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Fakta kunci terbaru dalam memori temporal'),
            h('span', { class: 'panel-count' }, `Total ${props.result.facts.length} item`)
          ]),
          ...
          }, expandedFacts.value ? `Tutup ▲` : `Buka semua ${props.result.facts.length} item ▼`)
        ]),

        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entitas utama'),
            h('span', { class: 'panel-count' }, `Total ${props.result.entities.length} item`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) =>
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount}item`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Tutup ▲` : `Buka semua ${props.result.entities.length} item ▼`)
        ]),

        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Rantai relasi'),
            h('span', { class: 'panel-count' }, `Total ${props.result.relations.length} item`)
          ]),
          ...
          }, expandedRelations.value ? `Tutup ▲` : `Buka semua ${props.result.relations.length} item ▼`)
        ]),

        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift Querydibuatanalisis Sub-pertanyaan'),
            h('span', { class: 'panel-count' }, `Total ${props.result.subQueries.length} item`)
          ]),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Belum ada Memori kunci saat ini'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Belum ada Entitas utama'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Belum ada Rantai relasi')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Saat iniaktifmemori (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Riwayatmemori (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entitas terkait (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Saat iniaktifmemori'),
            h('span', { class: 'panel-count' }, `Total ${props.result.activeFacts.length} item`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Belum ada Saat iniaktifmemori'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Tutup ▲` : `Buka semua ${props.result.activeFacts.length} item ▼`)
        ]),

        // Historical Facts Tab
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Riwayatmemori'),
            h('span', { class: 'panel-count' }, `Total ${props.result.historicalFacts.length} item`)
          ]),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Belum ada Entitas terkait'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(Platform ini belum memberi respons)' || t === '(Platform ini belum memberi respons)' || t === '[Tidak adarespons]'
    }
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        props.result.interviews[activeIndex.value]?.selectionReason && h('div', { class: 'selection-reason' }, [
          h('div', { class: 'reason-label' }, 'Pilih alasan'),
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      showTabs.value && h('div', { class: 'quicksearch-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `fakta (${props.result.facts.length})`)
        ]),
        hasEdges.value && h('button', {
          class: ['quicksearch-tab', { active: activeTab.value === 'edges' }],
          onClick: () => { activeTab.value = 'edges' }
        }, [
          h('span', { class: 'tab-label' }, `relasi (${props.result.edges.length})`)
        ]),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Carihasil'),
            h('span', { class: 'panel-count' }, `Total ${props.result.facts.length} item`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'tidakDitemukanterkaithasil'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Tutup ▲` : `Buka semua ${props.result.facts.length} item ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'terkaitrelasi'),
            h('span', { class: 'panel-count' }, `Total ${props.result.edges.length} item`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Node terkait'),
            h('span', { class: 'panel-count' }, `Total ${props.result.nodes.length} item`)
          ]),
...
const activeStep = computed(() => {
  ...
  // Catatan internal
  return steps[0] || { noLabel: '--', title: 'MenungguMulai ', status: 'todo', meta: '' }
})
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Catatan internal
...
const extractFinalContent = (response) => {
  ...
  // Catatan internal
  const chineseFinalMatch = response.match(/akhirjawaban[::]\s*\n*([\s\S]*)$/i)

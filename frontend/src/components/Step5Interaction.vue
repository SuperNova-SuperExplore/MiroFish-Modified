        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>Chat dengan Report Agent</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'Chat dengan agent simulasi' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">Pilih lawan bicara</div>
                <div
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || 'Profesi tidak diketahui' }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <span>Kirim survei ke dunia simulasi</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge Analisis akar sebab</div>
                    <div class="tool-desc">Membandingkan seed dunia nyata dan status simulasi, memakai memori global/lokal untuk analisis lintas waktu</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch Pelacakan panorama</div>
                    <div class="tool-desc">Menelusuri struktur graf untuk merekonstruksi jalur penyebaran event dan arus informasi</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">QuickSearch Pencarian cepat</div>
                    <div class="tool-desc">Query cepat berbasis GraphRAG untuk mengambil atribut node dan fakta spesifik</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InterviewSubAgent Wawancara virtual</div>
                    <div class="tool-desc">Wawancara otonom paralel dengan agent simulasi untuk mengambil opini dan state psikologis</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || 'Profesi tidak diketahui' }}</span>
                </div>
              </div>
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">Ringkasan</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Chat dengan Report Agent untuk menggali isi laporan' : 'Chat dengan agent simulasi untuk memahami sudut pandangnya' }}
              </p>
            </div>
            <div
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Pilih target survei</span>
                <span class="selection-count">Terpilih {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || 'Profesi tidak diketahui' }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">Pilih semua</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">Kosongkan</button>
              </div>
            </div>

            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Pertanyaan survei</span>
              </div>
              <textarea
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="Tulis pertanyaan untuk semua target terpilih..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">Hasil survei</span>
              <span class="results-count">{{ surveyResults.length }} balasan</span>
            </div>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || 'Profesi tidak diketahui' }}</span>
                  </div>
                </div>
...
const selectAgent = (agent, idx) => {
  ...
  addLog(`Pilih lawan bicara: ${agent.username}`)
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(`Gagal mengirim: ${err.message}`)
    chatHistory.value.push({
      role: 'assistant',
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('Pilih satu agent simulasi dulu')
  }

  addLog(`Kirim ke  ${selectedAgent.value.username} kirim: ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Penanya' : 'Kamu'}:${msg.content}`)
      .join('\n')
    prompt = `Berikut percakapan kita sebelumnya:\n${historyContext}\n\nPertanyaan baru gue adalah:${message}`
  }
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} sudah merespons`)
    } else {
      throw new Error('Tidak ada responsdata')
    }
  } else {
    throw new Error(res.error || 'Request gagal')
  }
...
const submitSurvey = async () => {
  ...
  addLog(`Kirim surveike ${selectedAgents.value.size} itemtarget...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        // Catatan internal
        let responseContent = 'Tidak ada respons'
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || 'Tidak ada respons'
          }
        } else if (Array.isArray(resultsDict)) {
          // Catatan internal
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || 'Tidak ada respons'
          }
        }
      ...
      addLog(`Menerima ${surveyResults.value.length} balasan`)
    } else {
      throw new Error(res.error || 'Request gagal')
    }
  } catch (err) {
    addLog(`surveiGagal mengirim: ${err.message}`)
  } finally {
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('laporandatamemuatselesai')
    }
  } catch (err) {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`memuat ${profiles.value.length} itemsimulasiitemagent`)
    }
  } catch (err) {
    addLog(`memuatsimulasiitemagentgagal: ${err.message}`)
  }
}

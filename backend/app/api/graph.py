...
@graph_bp.route('/seed/generate', methods=['POST'])
def generate_seed():
    """
    ...
    Request (JSON):
        {
            ...
            "num_queries": 8        // optional, default 8
        }
    ...
    try:
        ...
        num_queries = data.get('num_queries', 8)

        logger.info(f"=== Generating seeds for topic: {topic[:80]} ===")

        from ..services.seed_generator import SeedGeneratorService
        generator = SeedGeneratorService()
        ...
        logger.info(f"Seed generation complete: {len(result['seeds'])} files, "
                     f"{result['total_chars']:,} chars, {result['sources_count']} sources")

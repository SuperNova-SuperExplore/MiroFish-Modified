...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            # 同步对抗性Agent到配置中（profile_generator可能注入了额外Agent）
            existing_agent_ids = {ac.agent_id for ac in sim_params.agent_configs}
            for profile in profiles:
                if profile.source_entity_type == "adversarial_agent" and profile.user_id not in existing_agent_ids:
                    sim_params.agent_configs.append(AgentActivityConfig(
                        agent_id=profile.user_id,
                        entity_uuid="",
                        entity_name=profile.name,
                        entity_type="adversarial_agent",
                        activity_level=0.85,
                        posts_per_hour=2.0,
                        ...
                        sentiment_bias=0.0,
                        stance="contrarian" if "Devil" in profile.name else "observer",

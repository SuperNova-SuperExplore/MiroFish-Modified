...
【建议分析维度（可选但强烈推荐）】
- **立场演化追踪**: 分析Agent们的立场/态度是如何在模拟过程中变化的。
  是趋向一致（共识）还是越来越分化（极化）？哪些事件导致了立场转变？
- **跨平台对比**: 如果模拟涉及Twitter和Reddit两个平台，分析两个平台上
  讨论内容和态度的差异。同一个话题在不同平台的表现有何不同？
- **少数派声音**: 特别关注与多数意见不同的Agent（如Devil's Advocate、Fact-Checker），
  他们的反对意见是否揭示了多数派忽略的盲点？
...
【分析深度提示】
- 注意追踪Agent们的立场变化轨迹（从初始立场到最终立场的演化过程）
- 识别"意见领袖"和"易受影响者"的行为差异
- 如果存在反对派Agent，分析他们的对立观点是否被事实支撑"""
...
每个预测性结论必须附带置信度标签，基于模拟数据的支撑程度：

- 🟢 **[HIGH CONF]** — 80%以上的模拟Agent行为一致，多条证据交叉验证
- 🟡 **[MED CONF]** — 50-80%的Agent支持，但存在显著分歧或样本有限
- 🔴 **[LOW CONF]** — 不到50%的Agent支持，或信息样本不足，属推测性判断

使用格式：
```
🟢 [HIGH CONF] 具体预测内容（基于X个Agent的Y行为模式）
🟡 [MED CONF] 具体预测内容（基于有限样本，但趋势明显）
🔴 [LOW CONF] 具体预测内容（推测性判断，需进一步验证）
```

要求：
- 每个章节至少包含2-3个带置信度标签的预测
- 在章节末尾提供一个简短的「置信度小结」，列出本章核心预测及其置信度
- 不要全部标注为 HIGH，应有区分度
- 置信度必须基于检索到的模拟数据，不能凭空标注"""
...
4. 别忘了为每个预测性结论标注置信度（🟢/🟡/🔴），并在章节末尾提供置信度小结"""

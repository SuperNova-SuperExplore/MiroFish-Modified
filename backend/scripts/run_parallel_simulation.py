...
def create_model_pool(config: Dict[str, Any], use_boost: bool = False):
    """
    创建多模型池（Multi-Model Ensemble）

    通过使用不同的LLM来驱动不同的Agent，增加认知多样性。
    每个模型有不同的推理模式、词汇偏好和认知偏差，
    从而使模拟中的Agent行为更加多元化和真实。

    如果设置了 LLM_MODEL_POOL 环境变量，将创建多个模型实例；
    否则回退到单模型模式。

    OASIS 支持 List[BaseModelBackend] 参数，
    会自动将不同模型分配给不同的Agent。
    ...
    Returns:
        List[BaseModelBackend] 或单个 BaseModelBackend
    """
    model_pool_str = os.environ.get("LLM_MODEL_POOL", "").strip()

    if not model_pool_str:
        # 没有配置模型池，回退到单模型
        print("[模型池] 未配置 LLM_MODEL_POOL，使用单模型模式")
        return create_model(config, use_boost=use_boost)

    model_names = [m.strip() for m in model_pool_str.split(",") if m.strip()]

    if len(model_names) <= 1:
        print(f"[模型池] 只有1个模型，使用单模型模式: {model_names[0] if model_names else 'default'}")
        return create_model(config, use_boost=use_boost)
    ...
    for model_name in model_names:
        try:
            ...
            models.append(m)
            print(f"[模型池] ✔ 已加载: {model_name}")
        except Exception as e:
            print(f"[模型池] ✘ 加载失败: {model_name} - {e}")

    if not models:
        print("[模型池] 所有模型加载失败，回退到单模型")
        return create_model(config, use_boost=use_boost)

    if len(models) == 1:
        print(f"[模型池] 只有1个模型加载成功，使用单模型模式")
        return models[0]

    print(f"\n🧠 多模型集成已启用: {len(models)} 个不同的LLM")
    print(f"   模型列表: {', '.join(model_names[:len(models)])}")
    print(f"   Agent将获得不同的“思维方式”，增加认知多样性\n")
...
def get_active_agents_for_round(
    ...
    使用动态兴奋度机制：
    - 初始轮次（前5轮）有"新鲜感"加成，活跃度提高30%
    - 兴奋度随轮次缓慢衰减（模拟现实中话题热度下降）
    - 但不会低于基线水平
    """
    ...
    # 动态兴奋度机制 — 模拟话题热度随时间的自然变化
    # 前5轮：新鲜感加成（topic刚出来，大家都很感兴趣）
    # 之后逐渐衰减到基线水平（人们逐渐失去兴趣）
    # 但永远不会低于 0.7（总有核心关注者在讨论）
    excitement_decay_rate = 0.97  # 每轮衰减3%
    if round_num < 5:
        excitement = 1.3  # 初始新鲜感加成
    else:
        excitement = max(0.7, 1.3 * (excitement_decay_rate ** (round_num - 5)))
    ...
    for cfg in agent_configs:
        ...
        # 对抗性Agent有更高的基础活跃度（他们总是想发声）
        if cfg.get("entity_type") == "adversarial_agent":
            activity_level = min(1.0, activity_level * 1.2)
...
async def run_twitter_simulation(
    ...
    # Twitter 使用多模型池（如果可用）或通用 LLM 配置
    model_or_pool = create_model_pool(config, use_boost=False)
...
async def run_reddit_simulation(
    ...
    # Reddit 使用多模型池（如果可用）或加速 LLM 配置
    model_or_pool = create_model_pool(config, use_boost=True)
